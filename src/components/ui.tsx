...
export function Button({
  ...
  return (
    <button className={`rounded-md border px-3 py-2 text-sm font-medium ${toneClass}`} type={type}>
      {children}
    </button>
  );
}
...
export function EmptyState({ children }: { children: ReactNode }) {
  return <div className="rounded-md border border-dashed border-border p-6 text-sm text-muted">{children}</div>;
}
